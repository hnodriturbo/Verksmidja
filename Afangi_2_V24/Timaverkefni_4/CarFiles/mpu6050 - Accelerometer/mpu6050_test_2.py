...
#         return gyro
    ...
    def get_acceleration(self):
        accel_raw = self.read_mpu6050_regs(MPU_ACCEL_XOUTH_REG, 6)
        ax, ay, az = [int.from_bytes(accel_raw[i:i+2], 'big', signed=True) for i in range(0, 6, 2)]
        return ax, ay, az
    
    def get_gyroscope(self):
        gyro_raw = self.read_mpu6050_regs(MPU_GYRO_XOUTH_REG, 6)
        gx, gy, gz = [int.from_bytes(gyro_raw[i:i+2], 'big', signed=True) for i in range(0, 6, 2)]
        return gx, gy, gz
...
print("created the mpu instance of the class")

# who_am_i = mpu.read_mpu6050_reg(0x75)
# print("WHO_AM_I register:", hex(who_am_i))
