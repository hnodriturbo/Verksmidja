...
###########################################################
## This code is my effort in practicing my best coding   ##
##   skills. It's understandable, reusable and easily    ##
## readable and uses principles of clear code designing. ## 
###########################################################



###########################################################
 ## ----- Hope You Enjoy The Reading Of The Code  ----- ##
###########################################################
...
# time.localtime() don't work when esp32 is solo on car without computer connection
def start_log_session(log_file_path="logfile.log"):
    # Get the current time as a tuple
    current_time_tuple = time.localtime()  # MicroPython's time.localtime() provides a tuple
    
    # Format the time manually (MicroPython doesn't support strftime in the same way)
...
def log_message(message, log_file_path="logfile.log"):
    # Directly write the log messages, assuming logfile.log is at the root
    with open(log_file_path, "a") as log_file:
